...
class LineEdit(QLineEdit):
    ...
    def __init__(self, sz=None):
        super(LineEdit, self).__init__(str(sz))  # 父类输入文字内容
        self.setReadOnly(True)
...
class UserItem(QWidget):
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_Return:
            self._listWidgetItem.confirm_input()
    ...
    def contextMenuEvent(self, e):
        ...
        delUser.triggered.connect(self._listWidgetItem.del_self_from_list)  # 选中就会触发
...
class UserListItem(QListWidgetItem):
    ...
    def __init__(self, parent, usrId, name=DEFAULT_USER,
        ...
        self._id = usrId
        self._group = str(group)  # 这两个参数放在本类，其它参数放到子类
        self._parent = parent
    ...
    def setGroup(self, group):
        ...
        print
        'the group of user %s should be undated!' % self.getName()
...
class GroupItem(QWidget, QObject):
    ...
    def contextMenuEvent(self, e):  # 右键菜单
        ...
        adusr.triggered.connect(self._listWidgetItem.add_new_user)
        ...
        delGroup.triggered.connect(self._listWidgetItem.del_self_from_list)  # 选中就会触发
...
class GroupListItem(QListWidgetItem):
    ...
    def __init__(self, parent, name=DEFAULT_GROUP):
        super(GroupListItem, self).__init__(name, None,
                                            QListWidgetItem.UserType)  # parent给None才能正确的insertItem而不能addItem
    ...
    @pyqtSlot()
    def addNewUser(self):
        (ok, uit, git) = self._parent.add_scan(group=self.getName())  # 调用父类的添加用户，但是添加到本组下
        self._parent.setCurrentItem(uit)  # 选中当前增加的用户，这一步很关键。
        # 这样的另一个好处是执行一次item切换，保证之前输入的放弃操作--输入失焦。
        uit.get_widget().edit_info()  # 允许可以编辑
...
class GroupUserList(QListWidget):
    ...
    def removeUserItem(self, uit):
        ...
        git.del_scan(uit)  # 用户对应的组下的uit列表需要删除该用户
        self.takeItem(uit)
        del uit
    ...
    def removeGroupItem(self, git):
        ...
        if gName == DEFAULT_GROUP:
            print
            "Group %s can't be deleted!" % (gName)
            return False
        for uit in git.usrList:
            self.addUser(name=uit.get_name(), head=uit.getHead(),
        ...
        self.takeItem(git)
        self.groupDict.pop(git.get_name())
        del git
    ...
    def slotAddGroup(self, b):
        ...
        # 这样的另一个好处是执行一次item切换，保证之前输入的放弃操作--输入失焦。
        git.get_widget().edit_info()  # 允许可以编辑
